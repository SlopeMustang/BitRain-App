...
> **처음 열 때 한 단계가 더 필요합니다.** 아직 Apple 공증을 거치지 않아 「확인되지 않은 개발자」로 막힙니다. 실행 → 차단 경고에서 **「완료」** → **시스템 설정 › 개인정보 보호 및 보안** 에서 **「그래도 열기」** 를 누르십시오. 처음 한 번만 하면 됩니다. (macOS 15부터는 우클릭 → 열기로 넘어가지 않습니다.)
...
[**문제 신고**](../../issues/new?template=bug_report.yml)를 열어 주십시오. **진단 스냅샷을 함께 붙여 주시면** 원인을 훨씬 빨리 좁힐 수 있습니다.

**문제가 없어도 보내 주시면 도움이 됩니다.** [**동작 보고**](../../issues/new?template=compatibility_report.yml)는 「내 환경에서 이렇게 되더라」를 받는 자리입니다. 이 앱은 하드웨어 한 조합에서만 시험되었으므로, **된다는 사실 자체가 검증 데이터**입니다.
